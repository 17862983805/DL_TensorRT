...
extern "C" void get_mt_output(
            ...
            float* reg_buffer,
            float* obj_buffer);
// extern "C" void get_Transpose_output(const float* cls_data_in, const float* reg_data_in, const float* obj_data_in,
//             const int height, const int width,
//             const int cls_channels ,const int reg_channels, const int obj_channels,
//             float* cls_buffer_out, float* reg_buffer_out, float* obj_buffer_out);
...
float* blobFromImage(cv::Mat& img){
    ...
    int img_h = img.rows;
    int img_w = img.cols;
    std::vector<float> mean_rgb = {0.0, 0.0, 0.0};
    std::vector<float> std_rgb = {1.0, 1.0, 1.0};
...
int main(int argc, char** argv) {
    ...
    cout<<"pr_img.shape: "<<pr_img.cols<<", "<<pr_img.rows<<endl;
    ...
    string onnx_preprocess_txt = "../onnx_preprocess.txt";
    ...
    string pytorch_inference_txt = "../pytorch_inference_preprocess.txt";
    ...
    for (int i = 0; i < pr_img.total()*3; i++)
    {
        ...
        // if(diff == 1)cout<<i<<": "<<blob[i]<<", "<<onnx_preprocess[i]<<endl;
        if(diff > max_diff) {
            max_diff = diff;
        }
    ...
    void* transposeOutputBuffer[nbBindings];
    ...
    for (int i = 0; i < nbBindings; i++)
    {
        ...
        CHECK(cudaMalloc(&transposeOutputBuffer[i], totalSize * sizeof(float)));
    }
    ...
    CHECK(cudaMemcpyAsync(cudaOutputBuffer[inputIndex], blob,
                          ...
                          stream));
    CHECK(cudaMemcpyAsync(transposeOutputBuffer[inputIndex], blob,
                          mBindBufferSizes[inputIndex] * sizeof(float),
                          cudaMemcpyHostToDevice, 
                          stream));
    ...
    for(int i=0; i < num_stage; ++i){
        ...
        //     reg_channels, 
        //     obj_channels,
        //     (float*)transposeOutputBuffer[cls_output_blob_index[i]],
        //     (float*)transposeOutputBuffer[reg_output_blob_index[i]],
        //     (float*)transposeOutputBuffer[obj_output_blob_index[i]]
        // );
    }
    ...
    // for(int i = 0;i < nbBindings;++i){
    //     CHECK(cudaMemcpyAsync(transposeoutput[i], transposeOutputBuffer[i], mBindBufferSizes[i] * sizeof(float), cudaMemcpyDeviceToHost, stream));
    // }
