...
def preprocess_example_input(input_config):
    ...
    #resize
    h,w = one_img.shape[:2]
    max_long_edge = max(input_shape[2:])
    max_short_edge = min(input_shape[2:])
    scale_factor = min(max_long_edge / max(h, w),
                       max_short_edge / min(h, w))
    new_w = int(w * float(scale_factor) + 0.5)
    new_h = int(h * float(scale_factor) + 0.5)
    one_img = mmcv.imresize(one_img, (new_w,new_h))
    print("one_img.resize:",one_img.shape)
    ...
    divisor = 32
    pad_val = 0
    pad_h = int(np.ceil(one_img.shape[0] / divisor)) * divisor
    pad_w = int(np.ceil(one_img.shape[1] / divisor)) * divisor
    shape = (pad_h,pad_w)
    padding = (0, 0, shape[1] - one_img.shape[1], shape[0] - one_img.shape[0])
